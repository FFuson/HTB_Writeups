---
title: "Resolute"
description: "Writeups verificados de la máquina Resolute de Hack The Box"
"og:image": "https://rootea.es/og/resolute.svg"
"twitter:image": "https://rootea.es/og/resolute.svg"
---

# Resolute

<p className="machine-summary"><span className="prompt"><code>$ tldr</code></span> SMB password spray → WinRM → DnsAdmins DLL hijack</p>

<div className="machine-meta">

| · | · |
| --- | --- |
| Sistema operativo | Windows |
| Dificultad | Medio |
| Vector | <span className="vbadge vbadge-windows-privesc">Windows privesc</span> |
| IP | `10.10.10.169` |
| Fecha de retirada | 2020-05-30 |
| Skills | RPC Enumeration - Abusing querydispinfo CrackMapExec SMB Authentication Sprying Abusing WinRM - EvilWinRM Information Leakage LOLBAS Abusing DnsAdmins Group - dnscmd [Privilege Escalation] Creating a malicious DLL and injecting it into the dns service |

</div>

## Writeups

| Idioma | Autor | Formato | Enlace |
| --- | --- | --- | --- |
| 🇪🇸 ES | **El Pingüino de Mario** | Vídeo | [Abrir](https://www.youtube.com/watch?v=ZDx3Y2RolZs) |
| 🇬🇧 EN | **0xdf** | Texto | [Abrir](https://0xdf.gitlab.io/2020/05/30/htb-resolute.html) |
| 🇬🇧 EN | **IppSec** | Vídeo | [Abrir](https://www.youtube.com/watch?v=8KJebvmd1Fk) |

## Recursos por skill

Documentación curada para cada técnica que aparece en la columna *Skills* de arriba. Fuentes: HackTricks, GTFOBins, PortSwigger, etc.

| Skill | Fuente | Enlace |
| --- | --- | --- |
| Information Leakage | HackTricks | [Abrir](https://book.hacktricks.wiki/en/pentesting-web/information-leak.html) |
| SMB (139/445) | HackTricks | [Abrir](https://book.hacktricks.wiki/en/network-services-pentesting/pentesting-smb/index.html) |

## Skills relacionadas que conviene dominar

EternalBlue (MS17-010) · MS08-067 (NetAPI)

## Si te gustó esta máquina, prueba

- [Heist](/htb/machines/windows/facil/heist) — Windows, Fácil
- [TimeLapse](/htb/machines/windows/facil/timelapse) — Windows, Fácil
- [Jerry](/htb/machines/windows/facil/jerry) — Windows, Fácil
- [Luke](/htb/machines/linux/medio/luke) — Linux, Medio
- [SneakyMailer](/htb/machines/linux/medio/sneakymailer) — Linux, Medio

---

## Comentarios y truquillos

¿Algo no funcionó del writeup oficial? ¿Has encontrado un truco mejor? Compártelo aquí — los comentarios viven en [GitHub Discussions](https://github.com/FFuson/HTB_Writeups/discussions).

<div className="rootea-giscus-wrap" data-giscus-term="machine:resolute" data-giscus-lang="es"></div>

_Última actualización: 2026-06-29_

<script type="application/ld+json">{`{"@context":"https://schema.org","@type":"TechArticle","headline":"Resolute — HTB Writeup Index","name":"Resolute","url":"https://rootea.es/htb/machines/windows/medio/resolute","inLanguage":"es","datePublished":"2020-05-30","dateModified":"2026-06-29","about":{"@type":"Thing","name":"Hack The Box (HTB)"},"keywords":"Information Leakage, SMB (139/445)","isPartOf":{"@type":"WebSite","name":"rootea.es","url":"https://rootea.es"},"author":{"@type":"Organization","name":"rootea.es"},"proficiencyLevel":"Medio"}`}</script>

<script type="application/ld+json">{`{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"Inicio","item":"https://rootea.es"},{"@type":"ListItem","position":2,"name":"Windows","item":"https://rootea.es/htb/machines/windows/medio/index"},{"@type":"ListItem","position":3,"name":"Medio","item":"https://rootea.es/htb/machines/windows/medio/index"},{"@type":"ListItem","position":4,"name":"Resolute","item":"https://rootea.es/htb/machines/windows/medio/resolute"}]}`}</script>
