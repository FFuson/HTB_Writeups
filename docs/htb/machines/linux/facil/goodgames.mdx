---
title: "GoodGames"
description: "Writeups verificados de la máquina GoodGames de Hack The Box"
"og:image": "https://rootea.es/og/goodgames.svg"
"twitter:image": "https://rootea.es/og/goodgames.svg"
---


<p className="machine-summary"><span className="prompt"><code>$ tldr</code></span> SQLi error → SSTI → docker breakout → pivot</p>

<div className="machine-meta">

| · | · |
| --- | --- |
| Sistema operativo | Linux |
| Dificultad | Fácil |
| Vector | <span className="vbadge vbadge-web">Web</span> |
| IP | `10.10.11.130` |
| Fecha de retirada | 2022-02-23 |
| Skills | SQLI (Error Based) Hash Cracking Weak Algorithms Password Reuse Server Side Template Injection (SSTI) Docker Breakout (Privilege Escalation) [PIVOTING] |

</div>

## Writeups

| Idioma | Autor | Formato | Enlace |
| --- | --- | --- | --- |
| 🇪🇸 ES | **S4vitar** | Vídeo | [Abrir](https://www.youtube.com/watch?v=r3WMeRtwmFc) |
| 🇪🇸 ES | **El Pingüino de Mario** | Vídeo | [Abrir](https://www.youtube.com/watch?v=qLeeLRn9Z78) |
| 🇬🇧 EN | **0xdf** | Texto | [Abrir](https://0xdf.gitlab.io/2022/02/23/htb-goodgames.html) |
| 🇬🇧 EN | **IppSec** | Vídeo | [Abrir](https://www.youtube.com/watch?v=0AafRQIaWmQ) |

## Recursos por skill

Documentación curada para cada técnica que aparece en la columna *Skills* de arriba. Fuentes: HackTricks, GTFOBins, PortSwigger, etc.

| Skill | Fuente | Enlace |
| --- | --- | --- |
| SSTI (Server-Side Template Injection) | PortSwigger | [Abrir](https://portswigger.net/web-security/server-side-template-injection) |
| Port Forwarding / Pivoting | HackTricks | [Abrir](https://book.hacktricks.wiki/en/generic-hacking/tunneling-and-port-forwarding.html) |
| SQL Injection | HackTricks | [Abrir](https://book.hacktricks.wiki/en/pentesting-web/sql-injection/index.html) |
| SQL Injection | PortSwigger | [Abrir](https://portswigger.net/web-security/sql-injection) |

## Skills relacionadas que conviene dominar

Remote Code Execution (RCE) · Cross-Site Scripting (XSS)

## Si te gustó esta máquina, prueba

- [Oz](/htb/machines/linux/dificil/oz) — Linux, Difícil
- [Toolbox](/htb/machines/windows/facil/toolbox) — Windows, Fácil
- [Catch](/htb/machines/linux/medio/catch) — Linux, Medio
- [Writer](/htb/machines/linux/medio/writer) — Linux, Medio
- [NunChucks](/htb/machines/linux/facil/nunchucks) — Linux, Fácil

---

## Comentarios y truquillos

¿Algo no funcionó del writeup oficial? ¿Has encontrado un truco mejor? Compártelo aquí — los comentarios viven en [GitHub Discussions](https://github.com/FFuson/HTB_Writeups/discussions).

<div className="rootea-giscus-wrap" data-giscus-term="machine:goodgames" data-giscus-lang="es"></div>

_Última actualización: 2026-08-24_

<script type="application/ld+json">{`{"@context":"https://schema.org","@type":"TechArticle","headline":"GoodGames — HTB Writeup Index","name":"GoodGames","url":"https://rootea.es/htb/machines/linux/facil/goodgames","inLanguage":"es","datePublished":"2022-02-23","dateModified":"2026-08-24","about":{"@type":"Thing","name":"Hack The Box (HTB)"},"keywords":"SSTI (Server-Side Template Injection), Port Forwarding / Pivoting, SQL Injection, SQL Injection","isPartOf":{"@type":"WebSite","name":"rootea.es","url":"https://rootea.es"},"author":{"@type":"Organization","name":"rootea.es"},"proficiencyLevel":"Fácil"}`}</script>

<script type="application/ld+json">{`{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"Inicio","item":"https://rootea.es"},{"@type":"ListItem","position":2,"name":"Linux","item":"https://rootea.es/htb/machines/linux/facil/index"},{"@type":"ListItem","position":3,"name":"Fácil","item":"https://rootea.es/htb/machines/linux/facil/index"},{"@type":"ListItem","position":4,"name":"GoodGames","item":"https://rootea.es/htb/machines/linux/facil/goodgames"}]}`}</script>
