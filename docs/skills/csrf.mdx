...
4. **PortSwigger** — [Bypassing SameSite cookie restrictions](/portswigger/labs/csrf/bypassing-samesite-restrictions-samesite-strict-bypass-via-cookie-refresh) (Practitioner) · 🇬🇧
5. **PortSwigger** — [CSRF where Referer validation depends on header being present](/portswigger/labs/csrf/bypassing-referer-based-defenses-referer-validation-depends-on-header-being-present) (Practitioner) · 🇬🇧
6. **PortSwigger** — [CSRF where token is duplicated in cookie](/portswigger/labs/csrf/bypassing-token-validation-token-duplicated-in-cookie) (Practitioner) · 🇬🇧
