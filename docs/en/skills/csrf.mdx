...
**Coverage:** 19 challenges across 3 platforms — 2 HackTheBox · 15 PortSwigger · 2 TryHackMe. 2 with a Spanish writeup, 2 with a video writeup.
...
4. **PortSwigger** — [Bypassing SameSite cookie restrictions](/en/portswigger/labs/csrf/bypassing-samesite-restrictions-samesite-strict-bypass-via-cookie-refresh) (Practitioner) · 🇬🇧
5. **PortSwigger** — [CSRF where Referer validation depends on header being present](/en/portswigger/labs/csrf/bypassing-referer-based-defenses-referer-validation-depends-on-header-being-present) (Practitioner) · 🇬🇧
6. **PortSwigger** — [CSRF where token is duplicated in cookie](/en/portswigger/labs/csrf/bypassing-token-validation-token-duplicated-in-cookie) (Practitioner) · 🇬🇧
