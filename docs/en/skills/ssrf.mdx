...
**Coverage:** 25 challenges across 3 platforms — 10 HackTheBox · 11 PortSwigger · 4 TryHackMe. 10 with a Spanish writeup, 18 with a video writeup.

## Where to start

Ordered by actual difficulty. At the same level: isolated lab first, then the guided room, and the full machine last.

1. **PortSwigger** — [Basic SSRF against another back-end system](/en/portswigger/labs/ssrf/basic-ssrf-against-backend-system) (Apprentice) · 🇬🇧 📹
2. **PortSwigger** — [Basic SSRF against the local server](/en/portswigger/labs/ssrf/basic-ssrf-against-localhost) (Apprentice) · 🇬🇧 📹
3. **PortSwigger** — [Blind SSRF with Shellshock exploitation](/en/portswigger/labs/ssrf/blind-shellshock-exploitation) (Apprentice) · 🇬🇧 📹
4. **PortSwigger** — [Exploiting XXE to perform SSRF attacks](/en/portswigger/labs/xxe/exploiting-xxe-to-perform-ssrf) (Apprentice) · 🇬🇧
5. **PortSwigger** — [SSRF with whitelist-based input filter](/en/portswigger/labs/ssrf/ssrf-with-whitelist-filter) (Apprentice) · 🇬🇧 📹
6. **TryHackMe** — [Advent of Cyber 2023](/en/tryhackme/rooms/adventofcyber2023) (Easy · ~1440 min) · 🇬🇧
